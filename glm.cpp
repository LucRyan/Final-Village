...
/*
	Stole from somewhere and did a little change... K
*/
GLubyte* 
glmReadBmp(char* filename)
{
	FILE* fp;
	long width, height;
	float biBitCount;
	...
	//skip structure BITMAPFILEHEADER
	fseek(fp, sizeof(BITMAPFILEHEADER),0);

	//put header info into BITMAPINFOHEADER head
	BITMAPINFOHEADER head;  
	fread(&head, sizeof(BITMAPINFOHEADER), 1,fp); 
	
	//get width, height, bit of every pixle
	width = head.biWidth;
	...
	image = new unsigned char[lineByte * height];
	fread(image,1,lineByte * height,fp);
	fclose(fp);
...
GLvoid glmLoadInVBO(GLMmodel* model)
{
  ...
  vec4* normals;
  normals = (vec4*)malloc(sizeof(vec4) * model->numPointsInVBO);
  counter = 0;
  // Copy to VBO normals
  for (int i = 0; i < model->numtriangles; i++) 
    {
      ...
      for (int j = 0; j < 3; j++) 
		{
		  ...
		  normals[counter] = vec4(x, y, z, 0);
		  counter++;
		}
    }
  // Reserve memory space for VBO texCoords
  vec2* texCoords;
  texCoords = (vec2*)malloc(sizeof(vec2) * model->numPointsInVBO);
  counter = 0;
  // Copy to VBO texcoords
  for (int i = 0; i < model->numtriangles; i++) 
  {
	  _GLMtriangle T = model->triangles[i];
	  for (int j = 0; j < 3; j++) 
  ...
  int sizeNormals = sizeof(GLfloat)*4*model->numPointsInVBO;
  int sizeTexcoords = sizeof(GLfloat)*2*model->numPointsInVBO;
  int sizeTotal   = sizePoints + sizeNormals + sizeTexcoords; 
  ...
  glBufferSubData( GL_ARRAY_BUFFER, sizePoints + sizeNormals, sizeTexcoords, texCoords );
...
GLvoid glmLoadGroupsInVBO(GLMmodel* model)
{
	int num = model->numgroups;
	GLMgroup* group = model->groups;
	GLuint *vao, *vbo;
	vao = (GLuint*)malloc(sizeof(GLuint) * num);
	vbo = (GLuint*)malloc(sizeof(GLuint) * num);

	for ( int i = 0; i < num; ++i ) {
		int numPointsInVBO = 3 * group->numtriangles;
		// Reserve memory space for VBO vertices
		vec4* points;
		points = (vec4*)malloc(sizeof(vec4) * numPointsInVBO);
		int counter = 0;
		// Copy to VBO vertices
		float x, y, z;
		for ( int i = 0; i < group->numtriangles; ++i ) 
		{
			_GLMtriangle T = model->triangles[group->triangles[i]];
			for (int j = 0; j < 3; j++) 
			{
				x = model->vertices[3*T.vindices[j] +0];
				y = model->vertices[3*T.vindices[j] +1];
		...
		// Reserve memory space for VBO normals
		vec4* normals;
		normals = (vec4*)malloc(sizeof(vec4) * numPointsInVBO);
		counter = 0;
		// Copy to VBO normals
		for (int i = 0; i < group->numtriangles; i++) 
		{
		...
		// Reserve memory space for VBO texCoords
		vec4* texCoords;
		texCoords = (vec4*)malloc(sizeof(vec4) * numPointsInVBO);
		counter = 0;
		// Copy to VBO texcoords
		for (int i = 0; i < group->numtriangles; i++) 
		{
			...
			for (int j = 0; j < 3; j++) 
			{
				x = model->texcoords[2*T.tindices[j] +0];
				y = model->texcoords[2*T.tindices[j] +1];
				texCoords[counter] = vec4(x, y, 0, 0);
				counter++;
			}
		}
		// Create a vertex array object
		...
		int sizeNormals = sizeof(GLfloat)*4*numPointsInVBO;
		int sizeTexcoords = sizeof(GLfloat)*4*numPointsInVBO;
		int sizeTotal   = sizePoints + sizeNormals + sizeTexcoords; 
...
GLvoid
glmDrawGroupsVBO(GLMmodel* model)
{
	int num = model->numgroups;
	GLMgroup* group = model->groups;
	for ( int i = 0; i < num; ++i ) {
